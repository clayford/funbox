...
#' @param use_palette Logical. TRUE to use color lb_palette. Use with 7 groups
#'   or less. Default is FALSE. The lb_palette consists of the following colors:
#'   \code{c("blue4", "darkorange", "goldenrod1", "gray62", "peachpuff", "peru", "slategray2")}
#' @return A ggplot2 theme that can be added to a ggplot2 object
#'
#' @seealso
...
#' p1 + theme_lb()
#'
#' p2 <- ggplot(mtcars, aes(mpg, wt, color = factor(am))) + geom_point()
#' p2 + labs(color = "AM") + theme_lb(use_palette = TRUE)
#' @export
theme_lb <- function(title_face = "bold",
                           title_size = 12,
                           axis_color = "black",
  ...
  lb_palette <- c("blue4", "darkorange", "goldenrod1", "gray62",
  ...
  if (use_palette) {
    return(list(theme,
                ggplot2::scale_color_manual(values = lb_palette),
                ggplot2::scale_fill_manual(values = lb_palette)))
  } else {
    return(theme)
  }
}
